...
vim.keymap.set({ "n", "v" }, "<leader>d", [["_d]], { desc = "Delete current line without storing it in the copy register" })
...
vim.keymap.set({ "n", "v" }, "<leader>y", [["+y]], { desc = "[Y]ank current line to system's clipboard" })
vim.keymap.set("n", "<leader>Y", [["+Y]], { desc = "[Y]ank current line's content to system's clipboard" })

vim.keymap.set("v", "↑", ":m '<-2<CR>gv=gv", { desc = "Move current selection up" })
vim.keymap.set("v", "↓", ":m '>+1<CR>gv=gv", { desc = "Move current selection down" })
