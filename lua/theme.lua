...
function M.configure(theme, hl, link)
    ...
    local ui = {
        ...
        bg2            = ({ "#121212", "#121212", "#F0F0F0" })[theme.index],
        bg3            = ({ "#1B1B1B", "#1B1B1B", "#EAEAEA" })[theme.index],
        bg4            = ({ "#2D2D2D", "#2D2D2D", "#DADADA" })[theme.index],
        fg1            = ({ "#D4D4D4", "#D4D4D4", "#000000" })[theme.index],
        fg2            = ({ "#ABB2BF", "#ABB2BF", "#343D30" })[theme.index],
        fg3            = ({ "#8B92A8", "#8B92A8", "#646D67" })[theme.index],
        fg_dim         = ({ "#6B737F", "#6B737F", "#B4BCB0" })[theme.index],
        fg_disabled    = ({ "#545862", "#545862", "#CDCDCD" })[theme.index],
        sel1           = ({ "#083c5a", "#083c5a", "#BED3F7" })[theme.index],
        sel2           = ({ "#042e48", "#042e48", "#CCE1FB" })[theme.index],
        sel3           = ({ "#022036", "#022036", "#DEEFFD" })[theme.index],
        title          = ({ "#519FDF", "#519FDF", "#318FDF" })[theme.index],
        search         = ({ "#3E4451", "#3E4451", "#DFEFFF" })[theme.index],
        match          = ({ "#D8B43C", "#D8B43C", "#D8B43C" })[theme.index],
        match_dim      = ({ "#D5B06B", "#D5B06B", "#D5B06B" })[theme.index],
        cursor         = ({ "#00FF00", "#00FF00", "#FF00DD" })[theme.index],
        ...
        hint           = ({ "#66C9FF", "#66C9FF", "#50B0E0" })[theme.index],
        hint_light     = ({ "#A6D7F2", "#A6D7F2", "#90C0E0" })[theme.index],
        success        = ({ "#14C50B", "#14C50B", "#00AA00" })[theme.index],
        success_light  = ({ "#AAF2A6", "#AAF2A6", "#70DF70" })[theme.index],
    ...
    local syn = {
        ...
        punctuation   = ({ "#757473", "#757473", "#426969" })[theme.index],
        comment       = ({ "#7F775A", "#7F775A", "#A4A4A4" })[theme.index],
        ...
        mark_raw      = ({ "#CE9178", "#EE9977", "#BB6644" })[theme.index],
        mark_quote    = ({ "#6796E6", "#6796E6", "#0066EE" })[theme.index],
        mark_tag      = ({ "#569CD6", "#569CD6", "#0055AA" })[theme.index],
    ...
    -- Telescope
    hl("TelescopeSelection", ui.title, _)
